...
public class Imputacion implements Cargable{
	...
	public ArrayList<FraccionImputacion> mergeaListaFracciones(Proyecto p, ArrayList<FraccionImputacion> listaFracciones) {
	...
	public Cargable cargar(Object o) {
		...
		try { if (salida.get("impId")==null)     { this.id = 0;            } else this.id = (Integer) salida.get("impId");                           } catch (Exception e){System.out.println();}
		try { if (salida.get("impRecurso")==null)   { this.recurso = null;    } else {
		...
		try { if (salida.get("impProyecto")==null)   { this.proyecto = null;    } else {
		...
		try { if (salida.get("impSistema")==null)   { this.sistema = null;    } else {
			int idSistema = ((Integer) salida.get("impSistema"));
			this.sistema = Sistema.listado.get(idSistema);
		}} catch (Exception e){this.sistema = null;}
		try { if (salida.get("impGerencia")==null)   { this.sistema = null;    } else {
			int idGerencia = ((Integer) salida.get("impGerencia"));
			this.gerencia = MetaGerencia.porId(idGerencia);
		}} catch (Exception e){this.sistema = null;}
		try { if (salida.get("impNatCoste")==null)   { this.natCoste = null;    } else {
			int idnatcoste = ((Integer) salida.get("impNatCoste"));
			this.natCoste = MetaConcepto.porId(idnatcoste);
		}} catch (Exception e){this.sistema = null;}
		try { if (salida.get("impFxInicio")==null) { this.fxInicio = null;       } else this.fxInicio = (Date) FormateadorDatos.parseaDato((String)salida.get("impFxInicio"),FormateadorDatos.FORMATO_FECHA); } catch (Exception e){System.out.println();}
		try { if (salida.get("impFxFin")==null) { this.fxFin = null;       } else this.fxFin = (Date) FormateadorDatos.parseaDato((String)salida.get("impFxFin"),FormateadorDatos.FORMATO_FECHA); } catch (Exception e){System.out.println();}
		try { if (salida.get("impFxEnvioSAP")==null) { this.fxEnvioSAP = null;       } else this.fxEnvioSAP = (Date) FormateadorDatos.parseaDato((String)salida.get("impFxEnvioSAP"),FormateadorDatos.FORMATO_FECHA); } catch (Exception e){System.out.println();}
		try { if (salida.get("impHoras")==null)     { this.horas = 0;            } else this.horas = ((Double) salida.get("impHoras")).floatValue();    } catch (Exception e){System.out.println();}
		try { if (salida.get("impImporte")==null)     { this.importe = 0;            } else this.importe = ((Double) salida.get("impImporte")).floatValue();    } catch (Exception e){System.out.println();}
		try { if (salida.get("impTarifa")==null)   { this.tarifa = null;    } else {
			double idTarifa = ((Double) salida.get("impTarifa"));
			this.tarifa = Tarifa.porId(new Double(idTarifa).intValue());
		}} catch (Exception e){this.tarifa = null;}
		try { if (salida.get("impOT")==null)     { this.OT = "";            } else this.OT = (String) salida.get("impOT");                           } catch (Exception e){System.out.println();}
		try { if (salida.get("impPedido")==null)     { this.pedido = "";            } else this.pedido = (String) salida.get("impPedido");                           } catch (Exception e){System.out.println();}
		try { if (salida.get("impEstado")==null)     { this.estado = 0;            } else this.estado = (Integer) salida.get("impEstado");                           } catch (Exception e){System.out.println();}
		try { if (salida.get("impTipo")==null)   { this.tipoImputacion = Imputacion.IMPUTACION_NO_FRACCIONADA;    } else {
			this.tipoImputacion = ((Integer) salida.get("impTipo"));
		}} catch (Exception e){this.tipoImputacion = Imputacion.IMPUTACION_NO_FRACCIONADA;}
		
		return this;
	}
	
	public void insertImputacion()  throws Exception{
		...
		consulta.ejecutaSQL("iAltaimputacion", listaParms, this);
	}
	
	public void modificaImputacion()  throws Exception{
		...
		consulta = new ConsultaBD();
		consulta.ejecutaSQL("uActualizaImputacion", listaParms, this);
	}
	...
	public void borraImputacion()  throws Exception{
